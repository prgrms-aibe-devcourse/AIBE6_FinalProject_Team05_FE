...
//
// 홈(app/page.tsx)과 마켓(app/search/page.tsx)이 ① cardId→watchlistId Map ② 카드별 에러 문구
// ③ 로그인 확정 시 전체를 한 번 읽어오는 effect ④ 토글 핸들러를 글자 단위로 같은 코드로 각자
// 들고 있었다(#235). 하나만 고치면 다른 하나가 뒤처지는 상태라 여기로 합쳤다.
//
...
// 알아야 하고, 마켓은 하트가 자식 컴포넌트에 있다) handleHeartClick이 돌려주는 status를 보고
// 화면이 직접 처리한다.
export interface WatchlistErrorState {
  cardId: number;
  message: string;
}

// 등록/해제 실패 문구를 카드 옆에 잠깐 띄웠다가 지우는 시간.
const WATCHLIST_ERROR_CLEAR_MS = 3000;
...
export function useWatchlistMap() {
  ...
  const [watchlistError, setWatchlistError] = useState<WatchlistErrorState | null>(null);
  ...
  // 토글 결과 status를 그대로 돌려준다 — 토스트와 하트 펀치를 "서버가 등록을 확정한 뒤"에만
  // 재생하기 위해 호출부(화면)가 이 값을 보고 분기한다.
  //
  ...
  const handleHeartClick = async (cardId: number): Promise<QuickWatchlistToggleStatus> => {
    ...
    if (result.status === "added") {
      ...
      }, WATCHLIST_ERROR_CLEAR_MS);
    }
    return result.status;
  };

  return { myWatchlist, watchlistError, handleHeartClick, pendingCardId };
}
